...
// Adding Recharts for the Executive Analytics Dashboard
...
// --- NEW: Executive Risk Intelligence Data ---
const riskData = [
...
function App() {
  ...
  // 🔎 FORENSIC SCANNER LOGIC - UNTOUCHED
  ...
  useEffect(() => {
    ...
    // Bridge: Listen for live video stream from backend
  ...
  const reportIncident = async (type) => {
    try {
      await axios.post('http://localhost:5000/api/v1/alert', { 
        type: type, 
  ...
  const toggleVisionSystem = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/v1/toggle-vision', { 
        status: targetState 
      });
      if (response.data.success) {
        setIsVisionOn(targetState);
        if (!targetState) setLiveFrame(null); // Clear stream on standby
      }
  ...
  useEffect(() => {
    ...
    socket.on('security-alert', (data) => {
      // 🚨 NEW: DEAD-MAN SILENT DISTRESS LOGIC
      ...
      // Evidence photo capture from payload
  ...
  const handleScrub = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://localhost:5000/api/v1/scrub', {
        prompt: prompt,
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 ${visualAlert ? 'bg-slate-950' : 'bg-[#fcfdfe]'} p-4 md:p-8 font-sans`}>
      ...
      {/* 👻 NEW: SILENT ALARM INDICATOR (Only visible if Dead-man triggered) */}
      {isDeadManActive && (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* --- NEW: EXECUTIVE BUSINESS ANALYTICS --- */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          
          {/* LIVE FEED: Replacing Threat Graph with real-time camera if vision is on */}
        ...
        {/* TOP COMMAND CENTER - UNTOUCHED HEADER */}
        ...
        {/* CORE ANALYTICS TUNNEL - UNTOUCHED */}
        ...
        {/* 🔍 FORENSIC LEAK DETECTOR - UNTOUCHED */}
        ...
        {/* AUDIT COMPLIANCE MATRIX - UNTOUCHED */}
